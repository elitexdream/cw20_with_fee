...
pub const FEE_PERCENTAGE: &[u8] = b"fee_percentage";
...
pub fn instantiate(
    ...
    config_store.set(FEE_PERCENTAGE, &fee_percentage.to_be_bytes());
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetFeePercentage { fee_percentage } => try_set_fee_percentage(deps, &fee_percentage),
        ExecuteMsg::Withdraw { recipient, amount } => try_withdraw(deps, env, recipient, &amount)
    }
}
...
fn try_transfer(
    ...
    let config_store = PrefixedStorage::new(deps.storage, PREFIX_CONFIG);
    let data = config_store
        .get(FEE_PERCENTAGE)
        .expect("no fee percentage data stored");
    let fee_percentage = bytes_to_u128(&data).unwrap();
...
fn try_transfer_from(
    ...
    perform_transfer(deps.storage, _env, &owner_address, &recipient_address, amount_raw, fee_percentage)?;
...
fn try_set_fee_percentage(
    ...
    config_store.set(FEE_PERCENTAGE, &fee_percentage.to_be_bytes());
    Ok(Response::new()
        .add_attribute("config", "set_fee_percentage")
...
fn try_withdraw(
    ...
    let mut from_balance = match balances_store.get(contract_address.as_str().as_bytes()) {
    ...
    balances_store.set(contract_address.as_str().as_bytes(), &from_balance.to_be_bytes());

    let mut to_balance = match balances_store.get(recipient.as_str().as_bytes()) {
        Some(data) => bytes_to_u128(&data),
        None => Ok(0u128),
    }?;
    to_balance += amount_raw;
    balances_store.set(recipient_address.as_str().as_bytes(), &to_balance.to_be_bytes());
...
fn perform_transfer(
    ...
    let contract_address = _env.contract.address;
    let mut total_fee_amount = match balances_store.get(contract_address.as_str().as_bytes()) {
        Some(data) => bytes_to_u128(&data),
        None => Ok(0u128),
    }?;
    ...
    let fee_amount = fee_percentage * amount / FEE_DENOMINATOR;
    ...
    to_balance += amount_after_fee;
    balances_store.set(to.as_str().as_bytes(), &to_balance.to_be_bytes());
    balances_store.set(contract_address.as_str().as_bytes(), &total_fee_amount.to_be_bytes());
...
mod tests {
    ...
    fn get_fee_percentage(storage: &dyn Storage) -> u128{
        let config_storage = ReadonlyPrefixedStorage::new(storage, PREFIX_CONFIG);
        let data = config_storage
        .get(FEE_PERCENTAGE)
        .expect("no decimals data stored");
    ...
    mod transfer_with_fee {
        ...
        fn set_fee_percentage() {
            ...
            let (env, info) = mock_env_height("creator", 450, 550);
            let set_fee_msg = ExecuteMsg::SetFeePercentage { fee_percentage: Uint128::from(50u128) };
            let set_fee_result = execute(deps.as_mut(), env, info, set_fee_msg).unwrap();
            assert_eq!(
                set_fee_result.attributes,
                vec![
                    attr("config", "set_fee_percentage"),
                    attr("fee_percentage", "50")
                ]
            );
            
            let fee_percentage = get_fee_percentage(&deps.storage);
            assert_eq!(
                fee_percentage,
                50u128
            )
        ...
        fn can_send_with_fee() {
            ...
            assert_eq!(get_total_supply(&deps.storage), 60000);
            // Transfer
            let transfer_msg = ExecuteMsg::Transfer {
                recipient: "addr1111".to_string(),
        ...
        fn send_token_and_withdraw() {
            ...
            let (env, info) = mock_env_height("creator", 450, 550);
            let transfer_result = execute(deps.as_mut(), env, info, withdraw_msg).unwrap();
            assert_eq!(transfer_result.messages.len(), 0);
            assert_eq!(
                transfer_result.attributes,
                vec![
